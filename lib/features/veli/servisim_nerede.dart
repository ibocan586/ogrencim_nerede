...
class ServisimNeredePageVeli extends StatefulWidget {
  const ServisimNeredePageVeli({super.key});

  @override
  State<ServisimNeredePageVeli> createState() => _ServisimNeredePageVeliState();
}

class _ServisimNeredePageVeliState extends State<ServisimNeredePageVeli> {
  bool _loading = true;
  ...
  Future<void> _loadCarIcon() async {
    try {
      // Yeni API: BitmapDescriptor.asset
      // Eğer senin paket sürümünde yoksa paketi güncelle.
      ...
      // Hata olursa sessizce yoksay veya alternatif ikon koy
      if (!mounted) return;
      // fallback: null bırakıyoruz (default marker kullanılacak)
      debugPrint('Car icon yüklenemedi: $e');
    }
  }
  ...
  Future<void> _listenToDriverLocation() async {
    ...
    soforRef.snapshots().listen((doc) {
      ...
      if (konum != null && konum['lat'] != null && konum['lng'] != null) {
        // Güvenli dönüşüm: num -> double
        final lat = (konum['lat'] as num).toDouble();
        final lng = (konum['lng'] as num).toDouble();
  ...
  Future<void> _fitBothMarkers() async {
    ...
    await _googleMapController!.animateCamera(gmaps.CameraUpdate.newLatLngBounds(bounds, 80));
  }
        ...
        return Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              Text("Plaka: $plaka", style: const TextStyle(fontSize: 16, color: Colors.grey)),
              const SizedBox(height: 6),
              Text("Telefon: $phone", style: const TextStyle(fontSize: 16, color: Colors.grey)),
    ...
    final harita = kIsWeb
        ...
        : gmaps.GoogleMap(
            ...
            initialCameraPosition: gmaps.CameraPosition(
              target: gmaps.LatLng(_deviceLocation!.latitude, _deviceLocation!.longitude),
              zoom: 14,
            ),
            markers: {
              if (_carIcon != null)
                gmaps.Marker(
                  ...
                  position: gmaps.LatLng(_driverLocation!.latitude, _driverLocation!.longitude),
                  infoWindow: const gmaps.InfoWindow(title: "Servis Aracı"),
                  icon: _carIcon!,
                ),
              gmaps.Marker(
                markerId: const gmaps.MarkerId('me'),
                position: gmaps.LatLng(_deviceLocation!.latitude, _deviceLocation!.longitude),
        ...
        Positioned(
          ...
          child: Center(
            child: FloatingActionButton.extended(
              ...
              label: const Text("Şoför Bilgileri", style: TextStyle(color: Colors.white)),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text("Servis Nerede"),
        actions: [
          if (!_showPlakaForm)
            TextButton.icon(
